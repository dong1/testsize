...
#define PT_NODE_SR_IS_GLOBAL(node)		\
	((node)->info.serial.global_entity)
#define PT_NODE_SR_NODE_NAME(node)		\
	((node)->info.serial.on_node->info.value.text)
			   ...
			   const char *node_name)
{
  ...
  if (node_name)
    {
      is_global = 1;
      db_make_string (&value, node_name);
      error = dbt_put_internal (obj_tmpl, SR_ATT_NODE_NAME, &value);
      pr_clear_value (&value);
      if (error < 0)
	{
	  goto end;
  ...
  DB_MAKE_INT (&value, is_global);
  error = dbt_put_internal (obj_tmpl, SR_ATT_IS_GLOBAL, &value);
  pr_clear_value (&value);
  if (error < 0)
    {
      goto end;
  ...
  if (is_global)
    {
      ...
      DB_OBJECT *current_node, *cluster_node;
      ...
      if (OID_ISNULL (&temp_oid))
	{
      ...
      if (!ws_set_rmt_oid (ret_obj, &temp_oid, DB_CLUSTER_NODE_ALL))
	{
	  error = er_errid ();
	  goto end;
	}
      ...
      for (; node_list != NULL; node_list = node_list->next)
	{
	  ...
	  node_id = ccf_get_node_ip (cluster_node);
	  if (ws_set_remote (ret_obj, node_id) == false)
	    {
	      error = er_errid ();
	      goto end;
	    }
	  ...
	  /* reuse ret_obj */
	  obj_tmpl->object = ret_obj;
	  error = obt_update (obj_tmpl, &ret_obj);
	  if (error != NO_ERROR)
	    {
	      (void) ws_set_remote (ret_obj, DB_CLUSTER_NODE_LOCAL);
	      goto end;
	    }
	}
...
/*
 * global_serial_decache() - call serial_decache to all cluster node
 *   return: error code
 *  global_serial_obj(in): global serial object
 *  local_serial_oid(in): the OID of global serial on connected node
 *
 * Note: all remote OIDs should have already be saved into global_serial_obj
 */
static int
global_serial_decache (DB_OBJECT * global_serial_obj)
{
  DB_OBJECT *current_node, *cluster_node;
  DB_OBJLIST *node_list = ccf_get_node_list ();
  DB_IDENTIFIER *serial_oid;
  unsigned int node_id;
  int error_code = NO_ERROR;

  /* serial_decache () always return NO_ERROR */
  serial_oid = WS_OID (global_serial_obj);
  (void) serial_decache (serial_oid, DB_CLUSTER_NODE_LOCAL);
  ...
  for (; node_list != NULL; node_list = node_list->next)
      ...
      if (ws_mop_compare (current_node, cluster_node) == 0)
	{
      ...
      node_id = ccf_get_node_ip (cluster_node);
      if (ws_set_remote (global_serial_obj, (int) node_id) == false)
	{
	  error_code = er_errid ();
	  goto error;
	}
      serial_oid = ws_rmt_oid (global_serial_obj);
      /* serial_decache () always return NO_ERROR */
      (void) serial_decache (serial_oid, node_id);
    }

error:
  if (WS_IS_REMOTE (global_serial_obj))
    {
      ws_set_remote (global_serial_obj, DB_CLUSTER_NODE_LOCAL);
    }
...
/*
 * do_get_serial_node_id() -
 *   return: Error code
 *   node_id(out) : node id of serial
 *   serial_obj(in)  : MOP of serial
 *
 * Note: in CS_MODE, if serial node id is the same as connected node id,
 *       DB_CLUSTER_NODE_LOCAL will be the result.
 */
int
do_get_serial_node_id (int *node_id, MOP serial_obj)
{
  ...
  char *node_name;
  unsigned int node_ip, connected_node_id;
#endif

  error = db_get (serial_obj, SR_ATT_IS_GLOBAL, &is_global_val);
  if (error != NO_ERROR)
    {
      return error;
    }

  assert (DB_VALUE_TYPE (&is_global_val) == DB_TYPE_INTEGER);
  is_global = DB_GET_INT (&is_global_val);
#if defined(SA_MODE)
  if (is_global != 0)
    {
      error = ER_CLUSTER_OPERATION_IN_SA_MODE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error, 0);
      return error;
    }
  ...
  *node_id = DB_CLUSTER_NODE_LOCAL;
  ...
  error = db_get (serial_obj, SR_ATT_NODE_NAME, &node_name_val);
  if (error != NO_ERROR)
    {
      return error;
    }

  assert (DB_VALUE_TYPE (&node_name_val) == DB_TYPE_STRING);
  node_name = DB_GET_STRING (&node_name_val);
  node_ip = db_find_node_ip (node_name);
  if (!is_valid_ip (node_ip))
    {
...
do_create_serial (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  is_global = (PT_NODE_SR_IS_GLOBAL (statement) == PT_GLOBAL);
  if (is_global)
    {
      node_name = (char *) PT_NODE_SR_NODE_NAME (statement);
    }
...
do_alter_serial (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  (void) serial_decache ((OID *) (&serial_obj_id), DB_CLUSTER_NODE_LOCAL);
...
do_drop_serial (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  int found = 0, r = 0, save;
  bool au_disable_flag = false;
  bool is_global = (PT_NODE_SR_IS_GLOBAL (statement) == PT_GLOBAL);
  ...
  if (is_global)
    {
      ...
      db_make_string (&serial_name, name);
      serial_object =
	obj_find_oids_in_catalog_global (serial_class, SR_ATT_NAME,
					 &serial_name);
    }
  else
    {
      serial_object =
	do_get_serial_obj_id (&serial_obj_id, serial_class, name);
    }
  ...
  if (is_global)
    {
      error = obj_delete_global (serial_object);
    }
  else
    {
  ...
  if (is_global)
    {
      error = global_serial_decache (serial_object);
      if (error != NO_ERROR)
	{
	  goto end;
	}
    ...
    {
      (void) serial_decache (&serial_obj_id, DB_CLUSTER_NODE_LOCAL);
    }
...
do_statement (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  if (statement)
	...
	case PT_REGISTER_NODE:
	  error = do_register_node (parser, statement);
	  break;

	case PT_UNREGISTER_NODE:
	  error = do_unregister_node (parser, statement);
	  break;
#if 0
	case PT_SCHEMA_SYNC:
	  error = do_schema_sync (parser, statement);
	  break;
#endif
...
do_execute_statement (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  switch (statement->node_type)
    ...
    case PT_DO:
      ...
      break;
    case PT_REGISTER_NODE:
      err = do_register_node (parser, statement);
      break;
    case PT_UNREGISTER_NODE:
      err = do_unregister_node (parser, statement);
      break;
#if 0
    case PT_SCHEMA_SYNC:
      err = do_schema_sync (parser, statement);
      break;
#endif
    default:
      er_set (ER_ERROR_SEVERITY, __FILE__, statement->line_number,
		  ...
		  int has_uniques)
{
  ...
  if (xasl)
    {
      ...
      error = xts_map_xasl_to_stream (NULL, xasl, &stream, &size);
      if (error != NO_ERROR)
	{
	  PT_ERRORm (parser, statement, MSGCAT_SET_PARSER_RUNTIME,
  ...
  if (stream)
    {
      db_private_free_and_init (NULL, stream);
    }
      ...
      if (server_update)
	{
	  ...
	  if (!xasl_id && err == NO_ERROR)
	      ...
	      if (xasl && (err >= NO_ERROR))
		{
		  ...
		  err = xts_map_xasl_to_stream (NULL, xasl, &stream, &size);
		  if (err != NO_ERROR)
		    {
		      PT_ERRORm (parser, statement, MSGCAT_SET_PARSER_RUNTIME,
				 MSGCAT_RUNTIME_RESOURCES_EXHAUSTED);
			      ...
			      const bool has_default_values_list)
{
  ...
  if (!parser
      ...
      || !(class_ =
	   statement->info.insert.spec->info.spec.entity_name->info.
	   name.db_object))
    {
      return ER_GENERIC_ERROR;
    }
...
insert_local (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  for (row_count_total = 0; crt_list != NULL; crt_list = crt_list->next)
      ...
      else if (otemplate != NULL)
	{
	  ...
	  if (error >= NO_ERROR && !is_multiple_tuples_insert)
	    {
	      ...
	      if (vobj == NULL && locator_is_proxy_class (ws_class_mop (obj)))
		{
		  ws_decache (obj);
		  statement->etc = NULL;
		}
	  ...
	  /* when insert to remote server, decache the obj */
	  if (error >= NO_ERROR
	      && is_multiple_tuples_insert
	      && locator_is_proxy_class (ws_class_mop (obj)))
	    {
	      ws_decache (obj);
	    }
	}

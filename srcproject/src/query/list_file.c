  ...
  if (QFILE_GET_OVERFLOW_PAGE_ID (page_p) == NULL_PAGEID)
      ...
      do
	{
	  ...
	  if (page_p != first_page_p)
	    {
	      qmgr_free_old_page_proxy (thread_p, page_p, list_id_p);
	    }

	  if (ovfl_vpid.pageid != NULL_PAGEID)
	    {
	      page_p = qmgr_get_old_page_proxy (thread_p, &ovfl_vpid,
						list_id_p);
	      if (page_p == NULL)
		{
		  return ER_FAILED;
...
qfile_scan_next (THREAD_ENTRY * thread_p, QFILE_LIST_SCAN_ID * scan_id_p)
{
  ...
  if (scan_id_p->position == S_BEFORE)
    {
      /* In async mode, tuple_cnt is 0 */
      if (scan_id_p->list_id.tuple_cnt > 0
	  || scan_id_p->list_id.remote_node_id != 0)
	{
	  page_p =
	    qmgr_get_old_page_proxy (thread_p, &scan_id_p->list_id.first_vpid,
				     &scan_id_p->list_id);
	  if (page_p == NULL)
	    {
	      return S_ERROR;
	       ...
	       && (QFILE_GET_NEXT_PAGE_ID (page_p) != NULL_PAGEID_ASYNC))
	{
	  QFILE_GET_NEXT_VPID (&next_vpid, page_p);
	  next_page_p =
	    qmgr_get_old_page_proxy (thread_p, &next_vpid,
				     &scan_id_p->list_id);
	  if (next_page_p == NULL)
	    {
	      return S_ERROR;
	  ...
	  qmgr_free_old_page_proxy (thread_p, page_p, &scan_id_p->list_id);
...
end_scan:
  /* release last pages when data from remote. */
  if (scan_id_p->list_id.remote_node_id != 0
    && scan_id_p->list_id.last_pgptr)
    {
      free_and_init (scan_id_p->list_id.last_pgptr);
    }
return S_END;
}
...
/* qfile_add_tuple_to_val_list () - add one record from list file (scan_id->list_id)
 *									to val_list
 * return: SCAN_CODE (S_SUCCESS, S_END, S_ERROR)
 * type_list_p(in): DB_VALUE list
 * tuple(in): the tuple src.
 * dest_val_list(out):  the val_list will be filled.
 */
SCAN_CODE
qfile_add_tuple_to_val_list (QFILE_TUPLE_VALUE_TYPE_LIST * type_list_p,
			     QFILE_TUPLE tuple, VAL_LIST * dest_val_list)
{
  ...
  for (i = 0, valp = dest_val_list->valp;
       i < type_list_p->type_cnt; i++, valp = valp->next)
    {
      if (QFILE_GET_TUPLE_VALUE_FLAG (tuple_p) == V_BOUND)
	{
	  db_make_null (valp->val);
	  pr_type_p = type_list_p->domp[i]->type;
	  or_init (&buf, tuple_p + QFILE_TUPLE_VALUE_HEADER_SIZE,
		   QFILE_GET_TUPLE_VALUE_LENGTH (tuple_p));

	  (*(pr_type_p->readval)) (&buf, valp->val, type_list_p->domp[i],
				   -1, true, NULL, 0);
	}
      else
	{
	  ...
	  if (db_value_domain_init (valp->val, pr_type_p->id,
...
/* qfile_to_val_list () - scan one record from list file (scan_id->list_id)
 *									to val_list
 * return: SCAN_CODE (S_SUCCESS, S_END, S_ERROR)
 * thread_p(in): current thread
 * dest_val_list: DB_VALUE list
 * scan_id: scan identifier
 */
SCAN_CODE
qfile_to_val_list (THREAD_ENTRY * thread_p,
		   QFILE_LIST_SCAN_ID * scan_id, VAL_LIST * dest_val_list)
{
  QFILE_TUPLE_RECORD tuple_record = { NULL, 0 };
  SCAN_CODE qp_scan;
  ...
  if (qfile_add_tuple_to_val_list
      (&scan_id->list_id.type_list, tuple_record.tpl,
       dest_val_list) != NO_ERROR)
    {
      qfile_close_scan (thread_p, scan_id);
      return S_ERROR;
    }

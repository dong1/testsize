...
struct cls_spec_node
{
  ...
  int node_id;			/* cluster node identifier */
...
struct access_spec_node
{
  ...
  int flag;			/* flag for spec, see XASL_DO_NOT_REMOTE. */
...
struct update_proc_node
{
  ...
  int *node_ids;		/* the storage node id of each class    */
...
struct insert_proc_node
{
  ...
  OID proxy_oid;		/* the proxy class oid of connected server */
  HFID class_hfid;		/* Heap file ID of the class            */
  int node_id;			/* The node id of real table            */
  int no_vals;			/* total number of attrs involved       */
  int *att_id;			/* ID's of attributes (array)           */
...
struct xasl_node
{
  ...
  OUTPTR_LIST *remote_outptr_list;	/* outptr list for remote scan */
...
#define XASL_DO_NOT_REMOTE       256	/* do not remote */
#define XASL_EXEC_NODE           512	/* execution node */
#define XASL_DO_NOT_SCAN        1024	/* do not scan */
#define XASL_SPEC_VISITED       2048	/* spec has been visited */
...
struct xasl_parts_info
{
  ...
  OID proxy_class_oid;		/* the proxy class oid of connected server */
  HFID class_hfid;		/* Heap file ID of the sub-class */
  int node_id;			/* the node id of the sub-class */
};
...
extern int qexec_clear_list_cache_by_class (THREAD_ENTRY * thread_p,
					    const OID * class_oid);
extern int qexec_execute_insert (THREAD_ENTRY * thread_p, XASL_NODE * xasl,
...
#endif /* CUBRID_DEBUG */
extern int xts_map_xasl_to_stream (THREAD_ENTRY * thread_p,
				   const XASL_NODE * xasl, char **stream,
				   int *size);

extern void xts_final (THREAD_ENTRY * thread_p);
...
extern void qexec_replace_prior_regu_vars_prior_expr (THREAD_ENTRY * thread_p,
						      ...
						      connect_by_ptr);
/* 
 * struct query_exec_task_struct
 */
typedef struct query_exec_parm QUERY_EXEC_PARM;
struct query_exec_parm
{
  XASL_STATE *xasl_state;
  QUERY_FLAG flag;
};
...
/* 
 * macro for query_task
 *  
 *  QPARM_PTR   : get the query parameter object pointer
 *  QPARM_INIT  : init the query parameter object
 *  QPARM_FREE  : free the query parameter
 */

#if defined(SERVER_MODE)
#define QPARM_PTR(thread_p)        ((QUERY_EXEC_PARM *)((thread_p)->parm_ptr))
#else
extern QUERY_EXEC_PARM *query_exec_parm_ptr;
#define QPARM_PTR(thread_p)        (query_exec_parm_ptr)
#endif

#if defined(SERVER_MODE)
#define QPARM_INIT(thread_p)  do { (thread_p)->parm_ptr = qparm_open (thread_p); } while(0)
#else
#define QPARM_INIT(thread_p)  do { QPARM_PTR(thread_p) = qparm_open (thread_p); } while(0)
#endif

#if defined(SERVER_MODE)
#define QPARM_FREE(thread_p)   do { qparm_close ( thread_p, QPARM_PTR(thread_p)); \
                                     (thread_p)->parm_ptr = NULL; } while(0)
#else
#define QPARM_FREE(thread_p)   do { qparm_close ( thread_p, QPARM_PTR(thread_p)); \
                                     QPARM_PTR(thread_p) = NULL; } while(0)
#endif

extern int qparm_close (THREAD_ENTRY * thread_p,
			QUERY_EXEC_PARM * query_parm);
extern QUERY_EXEC_PARM *qparm_open (THREAD_ENTRY * thread_p);

extern int qexec_open_scans (THREAD_ENTRY * thread_p,
			     XASL_NODE * xasl,
			     XASL_STATE * xasl_state,
			     int fixed_scan_flag, int readonly_scan);

extern void qexec_close_scan (THREAD_ENTRY * thread_p,
			      ACCESS_SPEC_TYPE * curr_spec);

extern void qexec_end_scan (THREAD_ENTRY * thread_p,
			    ACCESS_SPEC_TYPE * curr_spec);

extern SCAN_CODE qexec_next_scan_block (THREAD_ENTRY * thread_p,
					XASL_NODE * xasl);

#if defined (SERVER_MODE)
extern int qexec_start_mainblock_iterations_proxy (THREAD_ENTRY * thread_p,
						   XASL_NODE * xasl,
						   XASL_STATE * xasl_state);
#else
#define qexec_start_mainblock_iterations_proxy \
...
#if defined (SERVER_MODE)
extern int scan_start_scan_proxy (THREAD_ENTRY * thread_p,
				  XASL_NODE * xasl, ACCESS_SPEC_TYPE * spec);
#else
#define scan_start_scan_proxy(thread_p, xasl, spec) \
        scan_start_scan (thread_p, &((spec)->s_id))
#endif

#if defined (SERVER_MODE)
extern int scan_next_scan_block_proxy (THREAD_ENTRY * thread_p,
				       ACCESS_SPEC_TYPE * spec);
#else
#define scan_next_scan_block_proxy(thread_p, spec)\
        scan_next_scan_block      (thread_p, &((spec)->s_id))
#endif

#if defined (SERVER_MODE)
extern int scan_reset_scan_block_proxy (THREAD_ENTRY * thread_p,
					XASL_NODE * xasl);
#else
#define scan_reset_scan_block_proxy(thread_p, xasl)\
        scan_reset_scan_block      (thread_p, &(xasl->curr_spec->s_id))
#endif

#if defined (SERVER_MODE)
extern int scan_next_scan_proxy (THREAD_ENTRY * thread_p,
				 XASL_NODE * exec_xasl,
				 ACCESS_SPEC_TYPE * spec, int direction,
				 bool count_star_with_iscan_opt);
#else
#define scan_next_scan_proxy(thread_p, xasl, spec, d, count_star_with_iscan_opt) (\
        (d) == S_FORWARD ? scan_next_scan      (thread_p, &((spec)->s_id))\
                       : scan_prev_scan      (thread_p, &((spec)->s_id)))
#endif

#if defined (SERVER_MODE)
extern void scan_end_scan_proxy (THREAD_ENTRY * thread_p,
				 XASL_NODE * xasl, ACCESS_SPEC_TYPE * spec);
#else
#define scan_end_scan_proxy(thread_p, xasl, spec)\
        scan_end_scan      (thread_p, &((spec)->s_id))
#endif

#if defined (SERVER_MODE)
extern void qexec_close_scan_proxy (THREAD_ENTRY * thread_p,
				    XASL_NODE * xasl,
				    ACCESS_SPEC_TYPE * spec);
#else
#define qexec_close_scan_proxy(thread_p, xasl, spec)\
        qexec_close_scan      (thread_p, spec)
#endif

extern int qexec_open_merge_scans (THREAD_ENTRY * thread_p,

		     ...
		     CACHE_TIME * server_cache_time_p)
{
  ...
  QPARM_INIT (thread_p);
  QPARM_PTR (thread_p)->flag = *flag_p;
  ...
  *query_id_p = query_p->query_id;
  ...
  if (!cached_result)
...
#if defined (SERVER_MODE)
      switch (query_p->xasl->type)
	{

	case DELETE_PROC:
	  if (NO_ERROR != remote_xqmgr_execute_delete_query (thread_p,
							     query_p->
							     query_id,
							     query_p->xasl,
							     dbvals_p,
	  ...
	  if (query_p->xasl->proc.delete_.no_classes == 0)
	    {
	...
	case UPDATE_PROC:
	  if (NO_ERROR != remote_xqmgr_execute_update_query (thread_p,
							     query_p->
							     query_id,
							     query_p->xasl,
							     dbvals_p,
	  ...
	  if (query_p->xasl->proc.update.no_classes == 0)
	    {
	      list_id_p->last_pgptr = NULL;
	      qmgr_mark_query_as_completed (query_p);
  ...
  if (cached_result)
      ...
      if (is_sync_query)
	{
	  ...
	  *flag_p &= ~ASYNC_EXEC;
	  ...
	  if (list_id_p)
	    {
	      query_p->list_id->tuple_cnt += list_id_p->tuple_cnt;
	    }
...
#if defined(SERVER_MODE)
CSS_CRITICAL_SECTION qparm_csect;
#endif

#if !defined (SERVER_MODE)
QUERY_EXEC_PARM *query_exec_parm_ptr = NULL;
#endif

/* 
 * qmgr_get_old_page_proxy(): the proxy function of qmgr_get_old_page.
 *                            for local page, just call qmgr_get_old_page directly.
 *                            for remote page, call remote_get_list_file_page to get page from romte.
 * return : the page ptr.
 * thread_p(in); current thread entry ptr;
 * vpid_p(in): the vpid of the page will get.
 * list_id_p(in): the list_id will used to get page.
 */
PAGE_PTR
qmgr_get_old_page_proxy (THREAD_ENTRY * thread_p, VPID * vpid_p,
			 QFILE_LIST_ID * list_id_p)
{
#if !defined(SERVER_MODE)

  return qmgr_get_old_page (thread_p, vpid_p, list_id_p->tfile_vfid);

#else

  int remote_node_id;
  remote_node_id = list_id_p->remote_node_id;

  /* if list for local table */
  if (remote_node_id == 0)
    {
      return qmgr_get_old_page (thread_p, vpid_p, list_id_p->tfile_vfid);
    }
  else
    {
      ...
      /* if last_pgptr include other pages, get page from last_pgptr */
      if (list_id_p->last_pgptr && list_id_p->remote_total_size > 0
	  && list_id_p->remote_offset < list_id_p->remote_total_size)
	{
	  page_p = list_id_p->last_pgptr + list_id_p->remote_offset;
	  list_id_p->remote_offset += DB_PAGESIZE;
	  return page_p;
	}
      ...
      if (list_id_p->last_pgptr)
	{
	  ...
	  list_id_p->remote_total_size = 0;
	  list_id_p->remote_offset = 0;
	}

      error = remote_get_list_file_page (thread_p,
					 remote_node_id,
					 list_id_p->query_id,
					 vpid_p->volid,
					 vpid_p->pageid,
					 &page_p, &total_page_size);
      if (error != NO_ERROR)
	{
	  /* error has been set */
	  return NULL;
      ...
      list_id_p->last_pgptr = page_p;
      list_id_p->remote_total_size = total_page_size;
      list_id_p->remote_offset = DB_PAGESIZE;
...
/* 
 * qmgr_free_old_page_proxy(): the proxy function of qmgr_free_old_page.
 *                            for local page, just call qmgr_free_old_page directly.
 * return : the page ptr.
 * thread_p(in); current thread entry ptr;
 * vpid_p(in): the vpid of the page will get.
 * list_id_p(in): the list_id will used to get page.
 */
void
qmgr_free_old_page_proxy (THREAD_ENTRY * thread_p, PAGE_PTR page_p,
			  QFILE_LIST_ID * list_id_p)
{
  QMGR_TEMP_FILE *tfile_vfid_p;
  int remote_id;
  ...
  remote_id = list_id_p->remote_node_id;
  if (remote_id == 0)
    {
 ...
 */
int
qparm_close (THREAD_ENTRY * thread_p, QUERY_EXEC_PARM * query_parm)
{
  ...
  db_private_free_and_init (thread_p, query_parm);
 ...
 */
QUERY_EXEC_PARM *
qparm_open (THREAD_ENTRY * thread_p)
{
  QUERY_EXEC_PARM *query_parm_ptr;

  query_parm_ptr =
    (QUERY_EXEC_PARM *) db_private_alloc (thread_p, sizeof (QUERY_EXEC_PARM));

  return query_parm_ptr;
}

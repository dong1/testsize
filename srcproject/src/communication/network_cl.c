...
/* net connection switching stack */
#define NET_SWITCHING_STACK_MAX   128
static int net_switching_stack[NET_SWITCHING_STACK_MAX] =
  { DB_CLUSTER_NODE_LOCAL };
static int net_switching_stack_top = 0;
...
/*
 * net_connection_switch_to_server -
 *
 * return: error code
 *
 *   storage_id(in):  the target server id, 
 *       DB_CLUSTER_NODE_LOCAL mean default host,
 *       other values mean the IP of the target server
 *
 * Note: This is to connect to a remote server via the connecting host.
 */

int
net_connection_switch_to_server (int storage_id)
{
  if ((net_switching_stack_top + 1) >= NET_SWITCHING_STACK_MAX)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
      return er_errid ();
    };

  if (storage_id != DB_CLUSTER_NODE_LOCAL)
    {
      if (ccf_get_node_ip (ccf_get_connected_node ()) ==
	  (unsigned int) storage_id)
	{
	  storage_id = DB_CLUSTER_NODE_LOCAL;
	}
    }

  if (net_switching_stack[net_switching_stack_top] != storage_id)
    {
      css_set_packet_route (css_Client_anchor->conn, storage_id);
    }

  net_switching_stack[++net_switching_stack_top] = storage_id;
 ...
 * returns error code.
 *
 * Note: Return to the previous server.
 */

int
net_connection_switch_back (void)
{
  if (net_switching_stack_top <= 0)
    {
      goto error;
    };

  if (NULL == css_Client_anchor)
    {
      goto error;
    }

  if (net_switching_stack[net_switching_stack_top] !=
      net_switching_stack[net_switching_stack_top - 1])
    {
      css_set_packet_route (css_Client_anchor->conn,
			    net_switching_stack[net_switching_stack_top - 1]);
    }

  --net_switching_stack_top;

...
static bool or_is_global_partition_parent_class (RECDES * record);
...
orc_diskrep_from_record (THREAD_ENTRY * thread_p, RECDES * record)
{
  ...
  if (or_class_type (record) == SM_PCLASS_CT
      || or_is_global_partition_parent_class (record))
    {
      /* on proxy class or global partition parent class, there's no
       * real index on them, so don't reading index information
       */
      or_rep = or_get_classrep_noindex (record, NULL_REPRID);
    }
  else
    {
...
/*
 * or_class_flags () - extracts the class flags from a record
 *
 *   return: class flags
 *   record(in): packed disk record containing class
 */
unsigned int
or_class_flags (RECDES * record)
{
  char *ptr;
  ...
  ptr =
    (char *) record->data +
    OR_FIXED_ATTRIBUTES_OFFSET (ORC_CLASS_VAR_ATT_COUNT) +
    ORC_CLASS_FLAGS_OFFSET;

  class_flags = (unsigned int) OR_GET_INT (ptr);

  return class_flags;
}

/*
 * or_class_type () - extracts the class type from a record
 *
 *   return: class type
 *   record(in): packed disk record containing class
 */
int
or_class_type (RECDES * record)
{
  char *ptr;
  int class_type;
  ...
  ptr =
    (char *) record->data +
    OR_FIXED_ATTRIBUTES_OFFSET (ORC_CLASS_VAR_ATT_COUNT) +
    ORC_CLASS_TYPE_OFFSET;

  class_type = OR_GET_INT (ptr);
...
/*
 * or_is_global_partition_parent_class () - get whether the class is the
 *                        parent class of global partition class
 *   return: true if class is global partition parent class, false else
 *
 *   record(in): disk record of class
 */
static bool
or_is_global_partition_parent_class (RECDES * record)
{
  int is_global;
  char *node_name;
  int class_type;

  is_global = or_class_flags (record) & SM_CLASSFLAG_GLOBAL;
  node_name = or_node_name (record);
  class_type = or_class_type (record);

  if (is_global && class_type == SM_CLASS_CT && *node_name == '\0')
    {
      return true;
    }

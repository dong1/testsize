...
/*
 * get_my_host_id () - get current host ip
 *   return: current host ip
 *   TODO: The host ip should NOT changed when CUBRID running
 */
unsigned int
get_my_host_id (void)
{
  char hostname[128];
  static unsigned int my_host_id = INADDR_NONE;

  if (my_host_id != INADDR_NONE)
    {
      return my_host_id;
    }
  ...
  if (gethostname (hostname, sizeof (hostname)) == 0)
    {
      my_host_id = get_host_main_ip (hostname);
      return my_host_id;
    }
  else
    {
...
unsigned int
get_host_main_ip (const char *hostname)
{
  struct hostent *he = NULL;
  unsigned int i, nIP;

  nIP = inet_addr (hostname);
  ...
  for (i = 0, nIP = 0; he->h_addr_list[i] != NULL; i++)
    {
      nIP = ntohl (*(unsigned int *) (he->h_addr_list[i]));

      if (nIP == INADDR_LOOPBACK)
	{
	  continue;
	}
...
/*
 * is_valid_ip() - return the result about whether the given IP is
 *                 valid for using
 * return : true if the IP is valid, false else
 * host_ip(in)  : the IP address
 */
bool
is_valid_ip (unsigned int host_ip)
{
  if (host_ip == INADDR_NONE || host_ip == INADDR_ANY
      || host_ip == INADDR_BROADCAST || host_ip == INADDR_LOOPBACK)
    {
      return false;
    }
  return true;

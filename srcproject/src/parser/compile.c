...
typedef struct pt_node_id_list PT_NODE_ID_LIST;
struct pt_node_id_list
{
  int num_nodes;
  int *node_ids;
};
...
static PT_NODE *pt_set_trigger_obj_post (PARSER_CONTEXT * parser,
					 PT_NODE * node, void *arg,
					 int *continue_walk);
static bool pt_is_include_node_id (PT_NODE_ID_LIST * id_list_p, int node_id);
...
/*
 * pt_is_include_node_id () - check whether the node id is in node id list.
 *   return: if found, return true;
 *   pt_node_id_list(in):
 *   node_id(in):
 */
static bool
pt_is_include_node_id (PT_NODE_ID_LIST * id_list_p, int node_id)
{
  int i;

  for (i = 0; i < id_list_p->num_nodes; i++)
    {
      if (node_id == id_list_p->node_ids[i])
	{
	  return true;
	}
    }

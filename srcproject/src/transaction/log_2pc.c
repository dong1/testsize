...
static int log_2pc_init_coord (LOG_TDES * tdes);
static int log_2pc_send_prepare_internal (int gtrid, int num_particps,
					  void *block_particps_ids);
static bool log_2pc_send_commit_internal (int gtrid, int num_particps,
					  int *particp_indices,
					  void *block_particps_ids);
static bool log_2pc_send_abort_internal (int gtrid, int num_particps,
					 int *particp_indices,
					 void *block_particps_ids,
					 int collect);
static int log_2pc_init_particp_ids (LOG_2PC_COORDINATOR * coord);
static LOG_2PC_PARTICIPANT *log_2pc_find_particp (int current_tran_index,
						  int host_id);


/*
 * global transaction id  :    H 16 bits ;  host_id
 *                             L 16 bits :  map index
 */
#define LOG_MAX_GLOBAL_ID_INDEX  1024
#define LOG_MAX_HOST_COUNT       64
struct log_tran_id_map
{
  int host_id;
  int map_to_local[LOG_MAX_GLOBAL_ID_INDEX];
};

typedef struct log_tran_id_map LOG_TRAN_ID_MAP;

/* TODO critical_section */
static int tran_id_map_len = 0;
static LOG_TRAN_ID_MAP tran_id_map[LOG_MAX_HOST_COUNT];
...
log_2pc_send_prepare (int gtrid, int num_particps, void *block_particps_ids)
{
  if (log_2pc_Userfun.send_prepare == NULL)
    {
      return log_2pc_send_prepare_internal (gtrid, num_particps,
					    block_particps_ids);
    }
			      ...
			      int *particps_indices, void *block_particps_ids)
{
  ...
  if (log_2pc_Userfun.send_commit != NULL)
    ...
    {
      result = log_2pc_send_commit_internal (gtrid, num_particps,
					     particps_indices,
					     block_particps_ids);
    }
			     ...
			     bool collect)
{
  ...
  if (log_2pc_Userfun.send_abort != NULL)
    ...
    {
      result = log_2pc_send_abort_internal (gtrid, num_particps,
					    particps_indices,
					    block_particps_ids, collect);
    }
			     ...
			     bool * decision)
{
  ...
  if (*decision == true)
      ...
      if (tdes->state != TRAN_UNACTIVE_COMMITTED_WITH_CLIENT_USER_LOOSE_ENDS)
	{
	  ...
	  /* TODO: Maybe we need try send decison again to participants 
	   * whose ack_received is false. */
...
static LOG_TRAN_ID_MAP *
log_get_tran_map_by_host_id (int host_id, bool auto_grow)
{
  LOG_TRAN_ID_MAP *map_i, *map_end;
  int i;
  map_i = tran_id_map;
  map_end = map_i + tran_id_map_len;

  for (; map_i != map_end; map_i++)
    {
      if (map_i->host_id == host_id)
	{
	  return map_i;
	}
    }

  if (tran_id_map_len >= LOG_MAX_HOST_COUNT)
    {
      return NULL;
    }
  ...
  TRAN_ID_MAP_CS_ENTER (thread_get_thread_entry_info ());

  map_i = tran_id_map + (tran_id_map_len++);

  map_i->host_id = host_id;

  for (i = 0; i < LOG_MAX_GLOBAL_ID_INDEX; i++)
    {
      map_i->map_to_local[i] = NULL_TRAN_INDEX;
    }
...
/*
 * log_make_global_tran_id () - 
 * return : a new global transaction id
 *  
 *  local_tran_id (in):
 */
int
log_make_global_tran_id (int local_tran_id)
{
#if defined(SERVER_MODE)
  LOG_TRAN_ID_MAP *map;
  int i;
  static int my_host_id = 0;
  LOG_TDES *tdes;


  if (my_host_id == 0)
    {
      my_host_id = get_my_host_id () & 0xFFFF;
    }

  if (local_tran_id == NULL_TRAN_INDEX)
    {
      return local_tran_id;
    };

  tdes = LOG_FIND_TDES (local_tran_id);
  if (tdes == NULL)
    {
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
	      ER_LOG_UNKNOWN_TRANINDEX, 1, local_tran_id);
      return NULL_TRANID;
    }

  map = log_get_tran_map_by_host_id (my_host_id, true);
  ...
  for (i = 0; i < LOG_MAX_GLOBAL_ID_INDEX; i++)
    {
      if (map->map_to_local[i] == NULL_TRAN_INDEX)
	{
	  map->map_to_local[i] = local_tran_id;

	  tdes->global_tran_id = (my_host_id << 16) | (i & 0xFFFF);

	  TRAN_ID_MAP_CS_EXIT ();
	  return tdes->global_tran_id;
	}
    };
...
/*
 * log_free_global_tran_id () - 
 *
 *  global_tran_id (in):
 */
void
log_free_global_tran_id (int global_tran_id)
{
  LOG_TRAN_ID_MAP *map;
  int global_tran_map_index;

  er_log_debug (ARG_FILE_LINE, "log_free_global_tran_id() is called !\n");

  if (global_tran_id == NULL_TRANID)
    {
      return;
    }

  global_tran_map_index = global_tran_id & 0xFFFF;

  if (global_tran_map_index >= LOG_MAX_GLOBAL_ID_INDEX)
    {
      return;
    }

  map =
    log_get_tran_map_by_host_id ((unsigned) (global_tran_id) >> 16, false);

  TRAN_ID_MAP_CS_ENTER (thread_get_thread_entry_info ());

  if (map)
    {
      map->map_to_local[global_tran_map_index] = NULL_TRAN_INDEX;
    }
...
/*
 * log_get_local_by_global_tran_id () -
 *
 ...
 */
int
log_get_local_by_global_tran_id (int global_tran_id)
{
  int local_tran_id = NULL_TRAN_INDEX;
#if defined(SERVER_MODE)
  LOG_TRAN_ID_MAP *map;
  int global_tran_map_index;
  static char program_name[] = "cub_server_s2s";
  ...
  if (global_tran_map_index >= LOG_MAX_GLOBAL_ID_INDEX)
    {
      er_log_debug (ARG_FILE_LINE, " max global id index exceeded."
		    " global_tran_id : %d, global_tran_map_index : %d \n",
		    global_tran_id, global_tran_map_index);
      return NULL_TRAN_INDEX;
    }

  map = log_get_tran_map_by_host_id ((unsigned) (global_tran_id) >> 16, true);

  if (map == NULL)
    {
      er_log_debug (ARG_FILE_LINE, "Didn't find tran map for host : %d\n",
		    global_tran_id >> 16);
      return NULL_TRAN_INDEX;
    }


  if (map->map_to_local[global_tran_map_index] != NULL_TRAN_INDEX)
    {
      return map->map_to_local[global_tran_map_index];
    }

  /* register client and register */

  memset (&client_credential, 0, sizeof (client_credential));
  memset (&server_credential, 0, sizeof (server_credential));

  client_credential.client_type = (int) BOOT_CLIENT_BROKER;
  client_credential.db_name = (char *) css_net_get_database_name ();
  client_credential.program_name = program_name;


  thread_p = thread_get_thread_entry_info ();
  local_tran_id = xboot_register_client (thread_p,
					 &client_credential,
  ...
  if (local_tran_id == NULL_TRAN_INDEX)
    {
      er_log_debug (ARG_FILE_LINE, "register_client failed!\n");
      return NULL_TRAN_INDEX;
    }

  map->map_to_local[global_tran_map_index] = local_tran_id;
  ...
  tdes->global_tran_id = global_tran_id;
#endif /* (SERVER_MODE) */
  return local_tran_id;
}

/*
 * log_2pc_add_participant () - Add a participant to the list. If the participant is already 
 * in the list, return the pointer directly.
 * return : the pointer of new participant
 *  
 *  thread_p(in) :
 *  host_id (in) : host_id of new participant
 */
LOG_2PC_PARTICIPANT *
log_2pc_add_participant (int current_tran_index, int host_id)
{
  LOG_TDES *tdes;		/* Transaction descriptor */
  LOG_2PC_PARTICIPANT *new_particp;

  tdes = LOG_FIND_TDES (current_tran_index);
  if (tdes == NULL)
    {
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
	      ER_LOG_UNKNOWN_TRANINDEX, 1, current_tran_index);
      return NULL;
    }

  /* TODO: need check host_id and tran_idx? */

  if (tdes->coord == NULL && log_2pc_init_coord (tdes) != NO_ERROR)
    {
      return NULL;
    }

  /* check if this host_id is already in current participant list. */
  if ((new_particp =
       log_2pc_find_particp (current_tran_index, host_id)) != NULL)
    {
      return new_particp;
    }

  if (tdes->coord->num_particps == LOG_2PC_MAX_NUM_PARTICP)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
	      ER_LOG_2PC_MAX_NUM_PARTICIPANT_EXCEEDED,
	      1, LOG_2PC_MAX_NUM_PARTICP);
      return NULL;
    }

  /* Add the host into participant list. */
  assert (tdes->coord->block_particps_ids != NULL);
  new_particp =
    &(((LOG_2PC_PARTICIPANT *) tdes->coord->block_particps_ids)[tdes->coord->
								num_particps]);
  new_particp->host_id = host_id;
  tdes->coord->num_particps++;
...
/*
 * log_2pc_find_particp () - Find a participant.
 * Return : the pointor if found, return NULL if not exist.
 *
 * thread_p (in)  :
 * host_id (in)   : host id of participant to find
 */
static LOG_2PC_PARTICIPANT *
log_2pc_find_particp (int current_tran_index, int host_id)
{
  LOG_2PC_PARTICIPANT *particp = NULL;
  LOG_TDES *tdes;
  int i;
  ...
  particp = (LOG_2PC_PARTICIPANT *) tdes->coord->block_particps_ids;
  for (i = 0; i < tdes->coord->num_particps; i++)
    {
      if (particp[i].host_id == host_id)
	{
	  return &particp[i];
	}
    }
...
/*
 * log_2pc_init_coord () - initialize coordinator.
 * return : error code
 *
 *  tdes (in) :
 */
static int
log_2pc_init_coord (LOG_TDES * tdes)
{
  int err = NO_ERROR;
  ...
  if (tdes == NULL)
    {
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
	      ER_LOG_UNKNOWN_TRANINDEX, 0);
      return er_errid ();
    }

  if (log_2pc_alloc_coord_info (tdes, 0, sizeof (LOG_2PC_PARTICIPANT *), NULL)
      == NULL)
    {
      err = er_errid ();
      goto mem_error;
  ...
  if (log_2pc_init_particp_ids (tdes->coord) != NO_ERROR)
    {
      err = er_errid ();
      goto mem_error;
    }
...
/*
 * log_2pc_init_particp_ids () - Initialize participant list of current g_transaction.
 * return : error code.
 *
 *  coord (in) :
 */
static int
log_2pc_init_particp_ids (LOG_2PC_COORDINATOR * coord)
{
  int i;
  size_t size;
  LOG_2PC_PARTICIPANT *particps;
  assert (coord != NULL);

  particps = (LOG_2PC_PARTICIPANT *) coord->block_particps_ids;
  if (particps != NULL)
    {
      free_and_init (particps);
    }

  size = sizeof (LOG_2PC_PARTICIPANT) * LOG_2PC_MAX_NUM_PARTICP;

  particps = (LOG_2PC_PARTICIPANT *) malloc (size);
  if (particps == NULL)
    {
  ...
  for (i = 0; i < LOG_2PC_MAX_NUM_PARTICP; i++)
    {
      particps[i].host_id = 0;
    }

  coord->block_particps_ids = (void *) particps;
  coord->num_particps = 0;

  return NO_ERROR;
}

/*
 * log_2pc_send_prepare_internal () - Send prepare command to every participant.
 * return : decision of this transaction.
 *
 *  gtrid (in) : global transaction index, be compatible with log_2pc_userfun.
 *  num_particps (in) :
 *  block_particps_ids (in) : participant list of this g_transaction.
 */
static int
log_2pc_send_prepare_internal (int gtrid, int num_particps,
			       void *block_particps_ids)
{
#if defined(SERVER_MODE)
  int decision;
  int i;
  TRAN_STATE state;
  LOG_2PC_PARTICIPANT *particps = (LOG_2PC_PARTICIPANT *) block_particps_ids;

  if (particps == NULL || num_particps < 0 || gtrid == LOG_2PC_NULL_GTRID)
    {
      //er_set ();
      return 0;
    }
  ...
  decision = 1;
  for (i = 0; i < num_particps && decision == 1; i++)
    {
      state = log_2pc_send_prepare_to_particp (&particps[i]);
	...
	{
	  er_log_debug (ARG_FILE_LINE,
			" couldn't get valid state from participant."
			" participant : %d,  state : %d\n",
			particps[i].host_id, state);
	  decision = 0;
	}
    }
  return decision;
...
/*
 * log_2pc_send_commit_internal () - Send commit command to participants who don't receive the command.
 * return : true if success, false otherwise.
 *
 *  gtrid (in) : global transaction index, be compatible with log_2pc_userfun.
 *  num_particps (in) :
 *  particp_indices (in) : flag to indicate whether the participant received decision.
 *  block_particps_ids (in) : participant list of this g_transaction.
 */
static bool
log_2pc_send_commit_internal (int gtrid, int num_particps,
			      int *particp_indices, void *block_particps_ids)
{
#if defined(SERVER_MODE)
  LOG_2PC_PARTICIPANT *particps = (LOG_2PC_PARTICIPANT *) block_particps_ids;
  ...
  if (num_particps < 0 || particp_indices == NULL || particps == NULL)
    {
      //er_set ();
      return false;
    }
  ...
  for (i = 0; i < num_particps; i++)
    {
      if (particp_indices[i] == 0)
	{
	  state = log_2pc_send_commit_to_particp (&particps[i]);
	  ...
	  if (state == TRAN_UNACTIVE_COMMITTED
	    ...
	    {
	      /* maybe log_2pc_append_recv_ack() should be called, but it's UNENABLE. */
	      particp_indices[i] = 1;
	    }
	  else
	    {
...
/*
 * log_2pc_send_abort_internal () - Send abort command to participants who don't receive the command.
 * return : true if success, false otherwise.
 *
 *  gtrid (in) : global transaction index, be compatible with log_2pc_userfun.
 *  num_particps (in) :
 ...
 */
static bool
log_2pc_send_abort_internal (int gtrid, int num_particps,
			     int *particp_indices, void *block_particps_ids,
			     int collect)
{
#if defined(SERVER_MODE)
  ...
  if (num_particps < 0
      || particps == NULL || (particp_indices == NULL && collect != 0))
    {
      //er_set ();
      return false;
    }
  ...
  for (i = 0; i < num_particps; i++)
    {
      if (particp_indices == NULL || particp_indices[i] == 0)
	{
	  state = log_2pc_send_abort_to_particp (&particps[i]);
	  ...
	  if (collect && ret)
	    {
	      /* log_2pc_append_recv_ack() should be called, but it's UNENABLE. */
	      er_log_debug (ARG_FILE_LINE,
			    " couldn't get valid state from participant."
			    " participant : %d,  state : %d\n",
			    particps[i].host_id, state);

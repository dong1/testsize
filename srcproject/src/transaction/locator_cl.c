...
{				/* Description of mflushing block structure */
  ...
  bool isone_mflush;		/* true, if we are not doing a
				 ...
				 */
  int node_id;			/* storage id */
  bool is_process_global;	/* true, if processing global class 
				   in the condition of which, 
				   only one object could be flushed at one time */
};
...
static LC_FIND_CLASSNAME locator_reserve_class_name (const char *class_name,
						     OID * class_oid);
static LC_FIND_CLASSNAME locator_reserve_class_name_global (const char
							    *class_name,
							    OID * class_oid);
static LC_FIND_CLASSNAME locator_delete_class_name_global (const char
							   *class_name);
static LC_FIND_CLASSNAME locator_rename_class_name_global (const char
							   *old_name,
							   const char
							   *new_name,
							   MOP class_mop);
static int locator_flush_class_global (MOP classop);
...
{
  return locator_reserve_class_names (1, &class_name,
				      class_oid, DB_CLUSTER_NODE_LOCAL);
}

/*
 * locator_reserve_class_name_global () -
 *    return:
 *  class_name(in):
 *  class_oid(in):
 */
static LC_FIND_CLASSNAME
locator_reserve_class_name_global (const char *class_name, OID * class_oid)
{
  ...
  while (node_list != NULL)
    {
      ...
      if ((reserved =
	   locator_reserve_class_names (1, &class_name, class_oid,
					ccf_get_node_ip (cur_node))) !=
	  LC_CLASSNAME_RESERVED)
	{
	  /* if fail, we may need call locator_delete_class_name() on reserved nodes */
	  /* if fail, we could rollback. */
	  break;
	}
...
/*
 * locator_delete_class_name_global () -
 *    return:
 *  class_name(in):
 *  class_oid(in):
 */
static LC_FIND_CLASSNAME
locator_delete_class_name_global (const char *class_name)
{
  ...
  while (node_list != NULL)
    {
      cur_node = node_list->op;
      if ((reserved =
	   locator_delete_class_name (class_name,
				      ccf_get_node_ip (cur_node))) !=
...
/*
 * locator_is_proxy_class() - Tests the class is a proxy class object.
 *   return: true or false
 *   op(in): class object
 */
bool
locator_is_proxy_class (MOP op)
{
  bool result = false;

  if (op != NULL
      && !locator_is_root (op)
      && locator_is_class (op, DB_FETCH_READ) && op->object != NULL)
    {
      result =
	((SM_CLASS *) op->object)->class_type == SM_PCLASS_CT ? true : false;
    }
...
locator_lock (MOP mop, LOCK lock)
{
  ...
  class_mop = ws_class_mop (mop);
  is_remote = WS_IS_REMOTE (mop);
  if (is_remote && object != NULL && class_mop != NULL)
    {
      is_remote_catainst = sm_is_system_class (class_mop) != 0;
    }
  ...
  if (object == NULL || current_lock == NULL_LOCK
	  ...
	  && !OID_ISTEMP (oid)) || (is_remote_catainst && !OID_ISTEMP (oid)))
    {
      ...
      if (object == NULL && WS_ISMARK_DELETED (mop))
	{
      ...
      node = WS_NODEID (mop);
      if (class_mop == NULL || (is_remote && !is_remote_catainst))
	{
      ...
      else
	{
      ...
      if (locator_fetch (oid, chn, lock, class_oid, class_chn,
			 is_prefetch, &fetch_area, node) != NO_ERROR)
	{
	  error_code = ER_FAILED;
	  goto error;
	}
...
#if defined(CS_MODE)
      /* If proxy class , then lock its real class */
      if (!is_remote && locator_is_proxy_class (mop))
	{
	  OID *real_oid;
	  int node_id;
	  object = (MOBJ) mop->object;

	  node_id = db_find_node_ip (sm_node_name (object));
	  real_oid = sm_real_oid (object);

	  if (locator_fetch (real_oid, chn, lock, class_oid, class_chn,
			     is_prefetch, &fetch_area, node_id) != NO_ERROR)
	    {
	      error_code = ER_FAILED;
	      goto error;
	    }
...
locator_lock_and_doesexist (MOP mop, LOCK lock)
{
  ...
  doesexist =
    locator_does_exist (oid, chn, lock, class_oid, class_chn, true,
			is_prefetch, &fetch_area, DB_CLUSTER_NODE_LOCAL);
  if (doesexist != LC_ERROR)
      ...
      if (locator_is_proxy_class (mop))
	{
	  ...
	  doesexist =
	    locator_does_exist (oid, chn, lock, class_oid, class_chn,
				true, is_prefetch, &fetch_area, node_id);
	  if (doesexist != LC_ERROR)
	    {
	      if (fetch_area != NULL)
		{
...
locator_fetch_class (MOP class_mop, DB_FETCH_MODE purpose)
{
  ...
  /* TODO: if the class is a global class and the purpose is WRITE 
   * then we should lock the real class and all proxy classes in cluster. */
			   ...
			   OID * class_oid, LOCK lock)
{
  ...
  *class_mop = NULL;
  found =
    locator_find_class_oid (classname, class_oid,
			    lock, DB_CLUSTER_NODE_LOCAL);
  switch (found)
    {
			   ...
			   bool isone_mflush)
{
  ...
  mflush->node_id = DB_CLUSTER_NODE_LOCAL;
  mflush->is_process_global = false;
...
locator_mflush_force (LOCATOR_MFLUSH_CACHE * mflush)
{
  ...
  if (mflush->mobjs->num_objs >= 0)
      ...
      while (mop_toid != NULL)
	{
      ...
      error_code = locator_force (mflush->copy_area, mflush->node_id);
      ...
      while (mop_toid != NULL)
	{
	  if (mop_toid->mop != NULL)
		...
		{
		  if (mflush->node_id != DB_CLUSTER_NODE_LOCAL)
		    {
		      if (mflush->node_id == WS_NODEID (mop_toid->mop)
			  && !ws_set_rmt_oid (mop_toid->mop, &obj->oid,
      ...
      if (error_code != NO_ERROR)
	{
	  ...
	  if (mflush->node_id == DB_CLUSTER_NODE_LOCAL)
	    ...
	    {
	      /* nothing ! Don't cache remote object. */
	    }
	}
    }
		       ...
		       WS_MAP_STATUS * map_status)
{
  ...
  save_old_chn = ((SM_CLASS *) object)->allow_new_chn;
	  ...
	  do
		  ...
		  != NO_ERROR)
		{
		  ...
		  *map_status = WS_MAP_FAIL;
	      ...
	      class_ = (SM_CLASS *) object;
	      class_->allow_new_chn = save_old_chn;
...
locator_mflush (MOP mop, void *mf)
{
   ...
   * buf if do schema sync, we don't care whether mop is dirty,
   * because this mop will be flush to remote node.
   */
  if (!WS_ISDIRTY (mop) && !sm_Do_schema_sync)
    {
      if (mflush->decache)
	{
	  if (WS_ISVID (mop))
  ...
  is_delete = (ws_find (mop, &object) == WS_FIND_MOP_DELETED);
  if (is_delete)
    {
      object = mop->object;
    }
  ...
  if (is_global_class && !mflush->is_process_global)
    {
      /* we should not process global table now. */
      return WS_MAP_CONTINUE;
    }
  ...
  if (WS_ISDIRTY (class_mop) && class_mop != mop)
    {
      ...
      if (sm_is_global_class (class_mop))
	{
	  bool is_delete = WS_ISMARK_DELETED (class_mop);
  ...
  if (is_delete)
      ...
      else
	{
	  ...
	  if (class_mop != mflush->class_mop)
	    {
	      /* should not enter this scope twice when process global object. */
	      ...
	      if (mflush->class_obj == NULL)
		{
	      ...
	      mflush->class_mop = class_mop;
	      /* what if the class is a global proxy class ? */
	      if (sm_is_system_class (class_mop) && WS_IS_REMOTE (mop))
		{
		  mflush->node_id = WS_NODEID (mop);
		}
      ...
      else
	{
	  ...
	  if (class_mop != mflush->class_mop)
	      ...
	      if (sm_is_system_class (class_mop) && WS_IS_REMOTE (mop))
		{
	      ...
	      else if (sm_get_class_type ((SM_CLASS *) mflush->class_obj) ==
		       SM_PCLASS_CT)
		{
		  mflush->node_id =
		    db_find_node_ip (sm_node_name (mflush->class_obj));
		  mflush->hfid = sm_real_hfid (mflush->class_obj);
		}
	      else
		{
		  mflush->hfid = sm_heap (mflush->class_obj);
  ...
  /* if a MOP is global, it shouldn't be changed. */
  if (!is_global_class && !WS_IS_REMOTE (mop))
    {
  ...
  if (is_global_class)
    {
      assert (mflush->mobjs->num_objs < 2);
      mflush->node_id = WS_NODEID (mop);
    }
...
/* locator_flush_class_global () flush a global class to all nodes.
 * return:
 *
 *  classop(in):
 */
static int
locator_flush_class_global (MOP classop)
{
  int err = NO_ERROR;
  MOP my_node, cur_node, real_node;
  DB_OBJLIST *node_list = ccf_get_node_list ();
  ...
  is_delete = (ws_find (classop, &object) == WS_FIND_MOP_DELETED);

  if (is_delete)
    {
      object = classop->object;
    }
  ...
  rnode_name = sm_node_name (object);
  my_node = ccf_get_connected_node ();
  real_node = rnode_name == NULL ? NULL : ccf_find_node (rnode_name);
  ...
  if (real_node != NULL)
      ...
      if (ws_mop_compare (real_node, my_node))
	{
	  ...
	  if (!ws_set_rmt_oid
	      (classop, sm_real_oid (object), DB_CLUSTER_NODE_NULL))
	    {
	      err = ER_FAILED;
	      goto end;
	    }
	  ...
	  HFID_COPY (sm_heap (object), sm_real_hfid (object));
	}

      err = locator_flush_class_helper (classop);

      HFID_SET_NULL (sm_heap (object));
      class_->class_type = SM_PCLASS_CT;
    }


  /* step 2. process other nodes except connected node. */

  while (node_list != NULL && err == NO_ERROR)
    {
      cur_node = node_list->op;
  ...
  if (WS_ISDIRTY (classop)
      && (ws_find (classop, &object) == WS_FIND_MOP_DELETED
	  || object != NULL))
    {
      if (is_delete)
	{
...
locator_flush_class (MOP class_mop)
{
  ...
  /* don't use sm_is_global_class() */
  if (((SM_CLASS *) class_mop->object)->flags & SM_CLASSFLAG_GLOBAL)
    {
      return locator_flush_class_global (class_mop);
    }
  else
    {
      return locator_flush_class_helper (class_mop);
    }
}

/*
 * locator_flush_class_helper () - Flush a dirty class
 *
 * return: NO_ERROR if all OK, ER status otherwise
 *
 *   class_mop(in): Mop of class to flush
 ...
 */
int
locator_flush_class_helper (MOP class_mop)
{
  ...
  class_obj = class_mop->object;
  if (class_obj != NULL
      && ((SM_CLASS *) class_obj)->flags & SM_CLASSFLAG_GLOBAL)
    {
      is_global = true;
    }
  /* When do schema sync, we want flush mop to remote node, so
   * we don't care whether the mop is dirty on current node
   */
  if ((WS_ISDIRTY (class_mop) || sm_Do_schema_sync)
      && (ws_find (class_mop, &class_obj) == WS_FIND_MOP_DELETED
	  || class_obj != NULL))
    {
      /*
      ...
      mflush.is_process_global = is_global;
      if (error_code == NO_ERROR)
	{
	  ...
	  if (map_status == WS_MAP_CONTINUE)
	    {
	      /* skip other dirty object if flush to other server */
	      map_status =
		is_global ? WS_MAP_SUCCESS : ws_map_dirty (locator_mflush,
							   &mflush);
...
retry:
  if (WS_ISDIRTY (inst_mop)
      ...
      if (error_code == NO_ERROR)
	{
	  ...
	  if (map_status == WS_MAP_CONTINUE)
	    {
	      /* skip local dirty object */
	      if (mflush.node_id != DB_CLUSTER_NODE_LOCAL)
		{
		  map_status = WS_MAP_SUCCESS;
		}
	      else
		{
...
locator_add_class (MOBJ class_obj, const char *classname)
{
  ...
  is_global = (((SM_CLASS *) class_obj)->flags & SM_CLASSFLAG_GLOBAL) != 0;
  if (!is_global)
    {
    ...
    {
      reserved =
	locator_reserve_class_name_global (classname, &class_temp_oid);
    }
  ...
  if (lock != NULL_LOCK)
      ...
      if (locator_lock (sm_Root_class_mop, IX_LOCK) != NO_ERROR)
	{
	  ...
	  if (!is_global)
	    ...
	    {
	      (void) locator_delete_class_name_global (classname);
	    }
	  return NULL;
	}
    }
  ...
  if (class_mop != NULL)
    {
      if (is_global
	  && !ws_set_rmt_oid (class_mop, &class_temp_oid,
			      DB_CLUSTER_NODE_ALL))
	{
	  ws_decache (class_mop);
	  class_mop = NULL;
	}
      else
	{
...
{
  MOBJ class_obj;		/* The class object */
  HFID *hfid = NULL;		/* Heap where instance will be placed */
  int node;
  int is_partition;
  bool is_global;
  ...
  node = WS_NODEID (class_mop);

  if (do_is_partitioned_classobj (&is_partition, class_mop, NULL, NULL) !=
      NO_ERROR)
    {
      return NULL;
    }

  is_global = sm_is_global_class (class_mop);
  if (is_global && is_partition == 1)
    {
      /* no need to create heapfile for a global parent. */
      return class_obj;
    }
  /*
  ...
  if (sm_get_class_type ((SM_CLASS *) class_obj) == SM_CLASS_CT)
    {
      hfid = sm_heap (class_obj);
    }

  if (hfid && HFID_IS_NULL (hfid))
    {
      OID *oid;
      ...
      if (OID_ISTEMP (oid))
	{
      ...
      if (heap_create (hfid, oid, reuse_oid, node) != NO_ERROR)
	{
	  return NULL;
	}
      ...
      if (sm_is_global_class (class_mop))
	{
	  HFID_COPY (sm_real_hfid (class_obj), hfid);
	}
...
locator_remove_class (MOP class_mop)
{
  ...
  is_global = sm_is_global_class (class_mop);
  is_global_super = is_global
    && ((SM_CLASS *) class_obj)->partition_of != NULL
    && ((SM_CLASS *) class_obj)->node_name == NULL;

  /* What should happen to the heap */
  if (is_global_super)
    {
      if (!HFID_IS_NULL (sm_heap (class_obj)))
	{
	  error_code = ER_FAILED;
	  goto error;
	}
    ...
    {
      insts_hfid =
	sm_get_class_type ((SM_CLASS *) class_obj) ==
	SM_PCLASS_CT ? sm_real_hfid (class_obj) : sm_heap (class_obj);

      nodeid =
	is_global ? db_find_node_ip (sm_node_name (class_obj)) :
	DB_CLUSTER_NODE_LOCAL;

      if (nodeid == INADDR_NONE)
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_CCF_NOT_FIND_NODE, 0);
	  error_code = er_errid ();
	  goto error;
	}
	...
	{
	  (void) heap_destroy_newly_created (insts_hfid, nodeid);
	}
    }
  ...
  if (is_global)
    {
      /* delay this step to locator_force() */
      /* deleted = locator_delete_class_name_global (classname); */
    }
  else
    {
      deleted = locator_delete_class_name (classname, DB_CLUSTER_NODE_LOCAL);
    }
			      ...
			      const char *new_classname)
{
  ...
  if (class_obj != NULL)
	...
	{
	  renamed =
	    locator_rename_class_name_global (old_classname,
					      new_classname, class_mop);
	}
      else
	{
	  renamed =
	    locator_rename_class_name (old_classname,
				       new_classname, ws_oid (class_mop),
				       DB_CLUSTER_NODE_LOCAL);
	}
...
/*
 * locator_rename_class_name_global -
 *
 * return:
 *
 *   old_name(in):
 ...
 */
static LC_FIND_CLASSNAME
locator_rename_class_name_global (const char *old_name, const char *new_name,
				  MOP class_mop)
{
  LC_FIND_CLASSNAME renamed = LC_CLASSNAME_ERROR;
  DB_OBJLIST *node_list, *cur_node;
  MOP my_node;
  int cur_nodeid;

  if (!sm_is_global_class (class_mop))
    {
      return locator_rename_class_name (old_name, new_name,
					WS_OID (class_mop),
					DB_CLUSTER_NODE_LOCAL);
    }
  ...
  for (cur_node = node_list; cur_node != NULL; cur_node = cur_node->next)
    {
      cur_nodeid = ws_mop_compare (my_node, cur_node->op) ?
	ccf_get_node_ip (cur_node->op) : DB_CLUSTER_NODE_LOCAL;

      if (!ws_set_remote (class_mop, cur_nodeid))
	{
	  renamed = LC_CLASSNAME_ERROR;
	  break;
	}
      renamed = locator_rename_class_name (old_name, new_name,
					   WS_OID (class_mop), cur_nodeid);
      if (renamed != LC_CLASSNAME_RESERVED_RENAME)
	{
	  break;
	}
  ...
  if (class_mop && WS_IS_REMOTE (class_mop))
    {
      ws_set_remote (class_mop, DB_CLUSTER_NODE_LOCAL);
    }
...
locator_assign_permanent_oid (MOP mop)
{
  ...
  bool is_remote;
  int node = DB_CLUSTER_NODE_LOCAL;
  ...
  if (locator_is_root (class_mop))
    ...
    {
      /* cannot assign oid to a remote object whose super is not root because doesn't know its super's OID */
      return NULL;
    }
  else
    {
  ...
  if (locator_assign_oid (hfid, &perm_oid, expected_length,
			  ws_oid (class_mop), name, node) != NO_ERROR)
    {
      if (er_errid () == ER_LK_UNILATERALLY_ABORTED)
	{
	  (void) tran_abort_only_client (false);
  ...
  if (object != NULL
      && sm_is_global_class (mop)
      && sm_get_class_type ((SM_CLASS *) object) == SM_CLASS_CT)
    {
      COPY_OID (sm_real_oid (object), &perm_oid);
    }

  if (is_remote)
    {
      ws_set_rmt_oid (mop, &perm_oid, DB_CLUSTER_NODE_NULL);
    }
  else
    {
			  ...
			  int quit_on_errors, int node)
{
  ...
  bool need_call_server;	/* Do we need to invoke the server to find
				 * the classes ? */
  bool need_flush;
  bool is_remote = (node != DB_CLUSTER_NODE_LOCAL);
  int error_code = NO_ERROR;
  int i;
   ...
   * shouldn't cache remote class. 
   */

  if (fetch_area != NULL)
    {
      /* Cache the classes that were brought from the server */
      if (node == DB_CLUSTER_NODE_LOCAL)
	{
  ...
  if (lockhint != NULL
	     ...
	     && lockhint->num_classes > lockhint->num_classes_processed)
	{
	  fetch_ptr[idx] = NULL;
	  error_code =
	    locator_fetch_lockhint_classes (lockhint, &fetch_ptr[idx], node);
	  if (error_code != NO_ERROR)
	    {
...
#if defined(CS_MODE)
  /* find lock hint real class */
  if (!is_remote && all_found == LC_CLASSNAME_EXIST)
    {
      LC_LOCKHINT *real_lock_hint;
      LC_COPYAREA *real_fetch_area;
      int chn;
      SM_CLASS *_class;
      int node_id;
      ...
      for (i = 0; i < num_classes; i++)
	{
	  if (many_classnames[i] &&
	      (class_mop = ws_find_class (many_classnames[i])) != NULL &&
	      locator_is_proxy_class (class_mop))
	    {
	      real_lock_hint = NULL;
	      real_fetch_area = NULL;

	      _class = (SM_CLASS *) class_mop->object;
	      chn = ws_chn (class_mop->object);
	      node_id = db_find_node_ip (_class->node_name);

	      all_found =
		locator_find_lockhint_class_oids (1,
						  &many_classnames[i],
						  &many_locks[i],
						  &need_subclasses[i],
						  &_class->real_oid, &chn,
						  quit_on_errors,
						  &real_lock_hint,
						  &real_fetch_area, node_id);

	      /* do not cache data of real class */
	      if (real_fetch_area != NULL)
		{
		  locator_free_copy_area (real_fetch_area);
		}


	      /* lock the rest of the objects and classes */
	      if (real_lock_hint != NULL
		  && (all_found == LC_CLASSNAME_EXIST
		      || quit_on_errors == false)
		  && real_lock_hint->num_classes >
		  real_lock_hint->num_classes_processed)
		{
		  LC_COPYAREA *fetch_area = NULL;

		  error_code = NO_ERROR;
		  error_code =
		    locator_fetch_lockhint_classes (real_lock_hint,
						    &fetch_area, node_id);

		  /* do not cache the data of real class */
		  if (fetch_area != NULL)
		    {
		      locator_free_copy_area (fetch_area);
		      fetch_area = NULL;
	      ...
	      if (real_lock_hint != NULL)
		{
		  locator_free_lockhint (real_lock_hint);
		}

	      if (all_found != LC_CLASSNAME_EXIST)
		{
		  goto error;
		}
	    }

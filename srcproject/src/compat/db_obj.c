...
db_find_unique (MOP classmop, const char *attname, DB_VALUE * value)
{
  ...
  retval = (obj_find_unique (classmop, attname, value, AU_FETCH_READ, NULL));
			   ...
			   const char *attname, DB_VALUE * value)
{
  ...
  retval = obj_find_unique (classmop, attname, value, AU_FETCH_UPDATE, NULL);
...
db_get_serial_current_value (const char *serial_name, DB_VALUE * serial_value)
{
  ...
  if (serial_mop != NULL)
      ...
      if (do_get_serial_cached_num (&cached_num, serial_mop) != NO_ERROR)
	{
      ...
      result = do_get_serial_node_id (&node_id, serial_mop);
      if (result != NO_ERROR)
	{
	  return result;
	}

      sprintf (oid_str, "%d %d %d %d %d", serial_obj_id.pageid,

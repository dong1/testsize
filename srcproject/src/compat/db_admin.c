...
/*
 * db_find_cluster_node() - find the cluster node object by node name
 * return : the node object if find the node, else return NULL
 * node_name(in)  : cluster node name
 */
DB_OBJECT *
db_find_cluster_node (const char *node_name)
{
  CHECK_CONNECT_NULL ();
  CHECK_1ARG_NULL (node_name);
  return ccf_find_node (node_name);
}

/*
 * db_get_node_ip() - get the cluster node IP  from the given
 *    cluster node object
 * return : the IP address of cluster node
 * node(in)  : cluster node objects
 */
unsigned int
db_get_node_ip (DB_OBJECT * node)
{
  CHECK_1ARG_RETURN_EXPR (node, INADDR_NONE);
  return ccf_get_node_ip (node);
}

/*
 * db_find_node_ip() - get the cluster node IP address from
 *    cluster node name
 * return : the IP address of cluster node. If not find the node, return
 *     INADDR_NONE
 * node_name(in)  : cluster node name
 */
unsigned int
db_find_node_ip (const char *node_name)
{
  DB_OBJECT *node;
  CHECK_CONNECT_AND_RETURN_EXPR (INADDR_NONE);
  CHECK_1ARG_RETURN_EXPR (node_name, INADDR_NONE);

  node = ccf_find_node (node_name);
  if (node == NULL)
    {
      return INADDR_NONE;
    }
...
/*
 * db_get_node_port() - get the cluster node port
 * return : port value if get it, else return -1
 * node_name(in)  : cluster node name
 */
int
db_get_node_port (DB_OBJECT * node)
{
  CHECK_1ARG_RETURN_EXPR (node, -1);
  return ccf_get_node_port (node);
}

/*
 * db_get_node_list() - get cluster node list which contains all
 *     cluster node object
 * return : cluster node object list
 */
DB_OBJLIST *
db_get_node_list (void)
{
  return ccf_get_node_list ();
}

/*
 * db_get_node_count() - get how many nodes in the cluster
 * return : node count
 */
int
db_get_node_count (void)
{
  return ccf_get_node_count ();;
}

/*
 * db_is_in_cluster() - get whether the connected DB is in cluster
 * return : boolean
 */
bool
db_is_in_cluster (void)
{
  if (db_get_node_count () > 0)
    {
      return true;
    }
  return false;
...
/*
 * db_get_connected_node() - get connected cluster node object
 * return : if connected DB in cluster, return the object of the connected node, else return NULL.
 */
DB_OBJECT *
db_get_connected_node (void)
{
  return ccf_get_connected_node ();
}

/*
 * db_get_connected_node_name() - get the node name of the connected cluster node
 * return : the connected node name
 */
const char *
db_get_connected_node_name (void)
{
  return ccf_get_connected_node_name ();
}

/*
 * db_register_cluster_node() - register a new node into cluster
 * return : error code
 ...
 */
int
db_register_cluster_node (const char *node_name,
			  const char *db_host, int cubrid_port)
{
  CHECK_CONNECT_ERROR ();
  CHECK_2ARGS_ERROR (node_name, db_host);
  CHECK_MODIFICATION_ERROR ();

  return ccf_register_node (node_name, db_host, cubrid_port);
}

/*
 * db_unregister_cluster_node() - unregister an exist node from cluster
 * return : error code
 * node_name(in) : the node name of the exist cluster node
 */
int
db_unregister_cluster_node (const char *node_name)
{
  CHECK_CONNECT_ERROR ();
  CHECK_1ARG_ERROR (node_name);
  CHECK_MODIFICATION_ERROR ();

  return ccf_unregister_node (node_name);
}

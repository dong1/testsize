...
static INTL_LANG cnv_currency_zone (DB_CURRENCY currency);
#if defined (ENABLE_UNUSED_FUNCTION)
...
cnv_currency_zone (DB_CURRENCY currency)
{
  ...
  INTL_LANG zone = INTL_LANG_ENGLISH;
  ...
  switch (currency)
    ...
    case DB_CURRENCY_DOLLAR:
      {
	zone = INTL_LANG_ENGLISH;
	break;
      }
    case DB_CURRENCY_WON:
      {
	zone = INTL_LANG_KOREAN;
	break;
      }
    case DB_CURRENCY_YUAN:
      {
	zone = INTL_LANG_CHINESE;
      }
    default:
      break;
    }

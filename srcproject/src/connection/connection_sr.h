...
extern void css_register_task_data_handler (void (*)
					    (const NET_HEADER *, CSS_BUFFER *,
					     THREAD_ENTRY **));
extern CSS_CONN_ENTRY *css_open_connection_to_remote_server (int tran_index,
							     int node_id,
							     unsigned short
							     *rid);
extern void css_close_connection_to_remote_server (CSS_CONN_ENTRY * conn);
extern void css_net_set_database_name (const char *db_name);
extern const char *css_net_get_database_name ();
extern CSS_BUFFER *css_alloc_buffer (size_t size);
extern void css_free_buffer (CSS_BUFFER * buff);

extern CSS_CONN_ENTRY *css_connect_to_remote_server (int node_id,
						     unsigned short *req_id,
						     bool create_new);

/*request a connection with pooled socket*/
extern CSS_CONN_ENTRY *s2s_begin_request (int node_id, int tran_index,
					  unsigned short *rid,
					  int wait_millisec);

/*free connection object and return the socket to pool*/
extern int s2s_end_request (CSS_CONN_ENTRY * conn);

extern int
css_receive_response (CSS_CONN_ENTRY * conn, unsigned short req_id,
		      char **buffer, int *buffer_size);
extern void s2s_free_conn_pooling (void);

...
typedef struct rmt_oid RMT_OID;
struct rmt_oid
{
  int node_id;
  OID oid;
...
struct db_object
{
  ...
  RMT_OID *rmt_oid;		/* remote oid array. */
  int rmt_node_idx;		/* specify current node index in rmt_oid array. */
...
#define WS_OID(mop) \
(WS_ISVID(mop) ? (OID *)&oid_Null_oid : (((mop)->rmt_node_idx == -1) ? &(mop)->oid_info.oid : ws_rmt_oid ((mop))))
#else /* SERVER_MODE */
#define WS_OID(mop) \
(WS_ISVID(mop) ? (OID *)&oid_Null_oid : (&(mop)->oid_info.oid))
#endif /* SERVER_MODE */

#define WS_NODEID(mop) \
  ((mop)->rmt_node_idx == -1 ? DB_CLUSTER_NODE_LOCAL : (mop)->rmt_oid[(mop)->rmt_node_idx].node_id)
#define WS_IS_REMOTE(mop)       (-1 != (mop)->rmt_node_idx)
...
extern bool ws_set_remote (MOP mop, int node_id);
extern bool ws_set_rmt_oid (MOP mop, OID * oid, int node_id);
extern OID *ws_rmt_oid (MOP mop);

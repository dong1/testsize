...
extern int obj_delete_global (MOP op);
...
extern MOP obj_find_unique (MOP op, const char *attname, DB_VALUE * value,
			    AU_FETCHMODE fetchmode, MOP inst_op);
...
extern int obj_find_partition_global (MOP op, SM_CLASS * class_,
				      MOP class_of);
extern MOP obj_find_oids_in_catalog_global (MOP catalog_class,
					    const char *att_name,
					    DB_VALUE * value);

		       ...
		       int level)
{
  ...
  if (error == NO_ERROR)
    {
      /* If insert data to proxy class, insert directly now */
      if (WS_IS_REMOTE (template_ptr->object)
	  || (template_ptr->class_ &&
	      template_ptr->object &&
	      sm_get_class_type (template_ptr->class_) == SM_PCLASS_CT))
	{
	  if (locator_flush_instance (template_ptr->object) != NO_ERROR)
	    {
	      error = er_errid ();
	    }

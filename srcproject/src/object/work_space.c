...
static void ws_insert_mop_on_hash_link_with_position (MOP mop, int slot,
						      MOP prev);
static bool ws_init_rmt (MOP mop);
      ...
      for (mop = ws_Mop_table[slot].head; mop != NULL; mop = mop->hash_link)
	{
	  ...
	  if (mop->pinned == 0
		  ...
		  || (((SM_CLASS *) mop->object)->class_type == SM_CLASS_CT
		      || ((SM_CLASS *) mop->object)->class_type ==
		      SM_PCLASS_CT)))
	    {
	      ws_decache (mop);
	      ws_clear_hints (mop, false);
 ...
 */
bool
ws_set_remote (MOP mop, int node_id)
{
  int i;
  CHECK_1ARG_FALSE (mop);

  if (node_id == DB_CLUSTER_NODE_NULL || node_id == DB_CLUSTER_NODE_LOCAL)
    {
      mop->rmt_node_idx = -1;
      return true;
    }

  if (node_id == DB_CLUSTER_NODE_ALL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_CCF_INVALID_HOST_ADDRESS,
	      0);
    }

  if (mop->rmt_oid == NULL && !ws_init_rmt (mop))
    {
      return false;
    }

  if (WS_NODEID (mop) == node_id)
    {
      return true;
    }

  for (i = 0; i < DB_MAX_NODE_IN_CLUSTER; i++)
    {
      if (mop->rmt_oid[i].node_id == node_id
	  || mop->rmt_oid[i].node_id == DB_CLUSTER_NODE_NULL)
	{
	  mop->rmt_node_idx = i;
	  mop->rmt_oid[i].node_id = node_id;
	  return true;
	}
    }

  if (i == DB_MAX_NODE_IN_CLUSTER)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
...
/*
 * ws_rmt_oid - return the corresponding oid on remote node.
 *    return: pointer to oid structure
 *    mop(in): object pointer
 *
 *  Note: The remote node is specified by mop->rmt_node_idx
 */
OID *
ws_rmt_oid (MOP mop)
{
  ...
  back_node_idx = mop->rmt_node_idx;
  rt_oid = &(mop)->rmt_oid[back_node_idx].oid;
  if (!OID_ISNULL (rt_oid))
    {
      goto end;
    }

  ws_set_remote (mop, DB_CLUSTER_NODE_LOCAL);
  if (locator_is_class (mop, DB_FETCH_READ))
    {
      if (WS_ISMARK_DELETED (mop))
	{
	  class_name = sm_classobj_name (mop->object);
	}
      else
	{
	  class_name = sm_class_name (mop);
	}
    }
  mop->rmt_node_idx = back_node_idx;

  if (class_name != NULL
      && LC_CLASSNAME_EXIST != locator_find_class_oid (class_name, rt_oid,
						       IS_LOCK,
						       WS_NODEID (mop)))
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
	      ER_LC_UNKNOWN_CLASSNAME, 1, class_name);
      COPY_OID (rt_oid, &oid_Null_oid);
    }
...
/*
 * ws_set_rmt_oid () - Set the remote oid value of mop specified by node_id.
 *  return : if success return true; otherwise return false.
 *
 *  mop(in) : 
 *  oid(in): remote oid value
 *  node_id(in): remote node id
 *
 * Note : when node_id is DB_CLUSTER_NODE_NULL, this function will use WS_NODEID(mop)
 * as remote node_id.
 */
bool
ws_set_rmt_oid (MOP mop, OID * oid, int node_id)
{
  int i;
  CHECK_2ARGS_ZERO (mop, oid);
  ...
  if (node_id == DB_CLUSTER_NODE_NULL || node_id == WS_NODEID (mop))
    {
      COPY_OID (WS_OID (mop), oid);
      return true;
    }
  ...
  if (node_id == DB_CLUSTER_NODE_ALL)
    {
      for (i = 0; i < DB_MAX_NODE_IN_CLUSTER; i++)
	{
	  COPY_OID (&mop->rmt_oid[i].oid, oid);
	}
      return true;
    }
...
/*
 * ws_init_rmt() - Initialize remote oid part
...
ws_init_rmt (MOP mop)
{
  ...
  size = sizeof (RMT_OID) * DB_MAX_NODE_IN_CLUSTER;
  mop->rmt_oid = (RMT_OID *) db_ws_alloc (size);
  if (mop->rmt_oid == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_OUT_OF_VIRTUAL_MEMORY,
	      1, size);
  ...
  for (i = 0; i < DB_MAX_NODE_IN_CLUSTER; i++)
    {
      OID_SET_NULL (&mop->rmt_oid[i].oid);
      mop->rmt_oid[i].node_id = DB_CLUSTER_NODE_NULL;
    }

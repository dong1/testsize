...
static MOP obj_find_object_by_cons_and_key (MOP classop,
					    ...
					    DB_VALUE * key,
					    AU_FETCHMODE fetchmode, MOP inst);
 ...
 *
 *  op(in):
 * NOte: only support delete on all nodes. */
int
obj_delete_global (MOP op)
{
  int error = NO_ERROR;
  SM_CLASS *class_ = NULL;
  DB_OBJECT *base_op = NULL;
  ...
  if (!class_->flags & SM_CLASSFLAG_SYSTEM)
    {
      /* only support instance of system class right now. */
      ERROR0 (error, ER_OBJ_INVALID_ARGUMENTS);
      goto error_exit;
    }

  /* this should be done on all node. */
  error = au_fetch_instance (op, &obj, AU_FETCH_UPDATE, AU_DELETE);
  if (error != NO_ERROR)
    {
      goto error_exit;
  ...
  for (; error == NO_ERROR && node_list != NULL; node_list = node_list->next)
    {
      ...
      if (ws_mop_compare (my_node, cur_node) == 0)
	{
      ...
      if (!ws_set_remote (op, ccf_get_node_ip (cur_node)))
	{
	  error = ER_FAILED;
	  goto error_exit;
	}

      error = au_fetch_instance (op, NULL, AU_FETCH_UPDATE, AU_DELETE);
      if (error != NO_ERROR)
	{
	  goto error_exit;
	}
    }
  ws_set_remote (op, DB_CLUSTER_NODE_LOCAL);
  /*
   * We don't need to decache the object as it will be decached when the mop
   * is GC'd in the usual way.
   */
  ...
  /* flush remote instance immediately. */
  ...
  for (; error == NO_ERROR && node_list != NULL; node_list = node_list->next)
    {
      cur_node = node_list->op;
      if (ws_mop_compare (my_node, node_list->op) == 0)
	{
	  continue;
	}
      if (!ws_set_remote (op, ccf_get_node_ip (cur_node)))
	{
      ...
      error = locator_flush_instance (op);
    }
...
/*
 * obj_find_oids_in_catalog_global () - find the corresponding instance in
 *              given system catalog. Save remote OID into the return MOP.
 * return: If success, return MOP of the instance in system catalog.
 *             Remote OID saved in the MOP.
 *         else, return NULL
 *
 *  catalog_name(in): the system class to find
 *  att_name(in): the attribute name to find
 *  value(in): value to look for
 *
 * NOTE: this function is for get all remote OIDs. On att_name,
 * there must contains an index. value must be unique on att_name.
 */
MOP
obj_find_oids_in_catalog_global (MOP catalog_class, const char *att_name,
				 DB_VALUE * value)
{
  MOP inst_in_catalog;
  MOP current_node, cluster_node;
  DB_OBJLIST *node_list;
  bool err_failed = false;

  if (value == NULL || catalog_class == NULL || att_name == NULL)
    {
      er_set (ER_WARNING_SEVERITY, ARG_FILE_LINE, ER_OBJ_INVALID_ARGUMENTS,
	      0);
      return NULL;
  ...
  if (!sm_is_system_class (catalog_class))
    {
      er_set (ER_WARNING_SEVERITY, ARG_FILE_LINE, ER_OBJ_INVALID_ARGUMENTS,
	      0);
      return NULL;
  ...
  inst_in_catalog =
    obj_find_unique (catalog_class, att_name, value, AU_FETCH_READ, NULL);
  if (inst_in_catalog == NULL)
    {
      return NULL;
    }

  node_list = ccf_get_node_list ();
  current_node = ccf_get_connected_node ();
  if (node_list == NULL || current_node == NULL)
    {
      return NULL;
    }
  ...
  for (; node_list != NULL; node_list = node_list->next)
    {
      cluster_node = node_list->op;
      if (ws_mop_compare (current_node, cluster_node) == 0)
	{
	  continue;
	}

      if (ws_set_remote (inst_in_catalog, ccf_get_node_ip (cluster_node)) ==
	  false)
	{
	  err_failed = true;
	  goto end;
      ...
      if (OID_ISNULL (WS_OID (inst_in_catalog)))
	{
	  if (obj_find_unique
	      (catalog_class, att_name, value, AU_FETCH_READ,
	       inst_in_catalog) == NULL)
	    {
	      err_failed = true;
	      goto end;
	    }
  ...
  if (WS_IS_REMOTE (inst_in_catalog))
    {
      ws_set_remote (inst_in_catalog, DB_CLUSTER_NODE_LOCAL);
    }

  return err_failed ? NULL : inst_in_catalog;
}

/*
 * obj_find_partition_global ()-
 * return  : error code
 *
 *  op(in):
 *  class_(in):*/
int
obj_find_partition_global (MOP op, SM_CLASS * class_, MOP class_of)
{
  int error = NO_ERROR;
  MOP partition_cata, part_of = NULL, tmp;
  const DB_VALUE *var_for_part[2];
  const char *varname_for_part[2];
  DB_VALUE var_pclassof, var_pname;
  MOP my_node, cur_node;
  DB_OBJLIST *node_list;
  int cur_nodeip;

  if (op == NULL || class_ == NULL || class_of == NULL
      || !sm_is_global_class (op) || !class_->partition_of)
    {
      ERROR0 (error, ER_OBJ_INVALID_ARGUMENTS);
      goto end;
    }
  ...
  part_of = class_->partition_of;
  partition_cata = sm_find_class (CT_PARTITION_NAME);
  if (partition_cata == NULL)
    {
      error = er_errid ();
      goto end;
    }

  varname_for_part[0] = PARTITION_ATT_CLASSOF;
  db_make_object (&var_pclassof, class_of);
  varname_for_part[1] = PARTITION_ATT_PNAME;
  error = db_get (part_of, PARTITION_ATT_PNAME, &var_pname);
  if (error != NO_ERROR)
    {
      goto end;
    }
  var_for_part[0] = &var_pclassof;
  var_for_part[1] = &var_pname;
  ...
  for (; node_list != NULL; node_list = node_list->next)
      ...
      if (ws_mop_compare (my_node, cur_node) == 0)
	{
      ...
      cur_nodeip = ccf_get_node_ip (cur_node);
      if (!ws_set_remote (part_of, cur_nodeip)
	  || !ws_set_remote (class_of, cur_nodeip))
	{
	  error = er_errid ();
	  goto end;
	}

      if (OID_ISNULL (WS_OID (part_of)))
	{
	  tmp = obj_find_multi_attr (partition_cata, 2,
				     varname_for_part, var_for_part,
				     AU_FETCH_UPDATE, part_of);
	  if (tmp == NULL)
	    {
	      error = er_errid ();
	      goto end;
  ...
  pr_clear_value (&var_pclassof);
  pr_clear_value (&var_pname);

  if (part_of && WS_IS_REMOTE (part_of))
    {
      ws_set_remote (part_of, DB_CLUSTER_NODE_LOCAL);
    }

  if (WS_IS_REMOTE (class_of))
    {
      ws_set_remote (class_of, DB_CLUSTER_NODE_LOCAL);
    }
 ...
 *                                 AU_FETCH_UPDATE
 *    op (in/out) : local instace. only used when fetch a remote
 *                  corresponding oid of this instance.
 *
 * Note:
...
find_unique (MOP classop, SM_ATTRIBUTE * att,
	     DB_VALUE * value, AU_FETCHMODE fetchmode, MOP op)
{
  MOP found = NULL;
  OID unique_oid;
  BTID btid;
  ...
  is_remote = (op == NULL ? false : WS_IS_REMOTE (op));
  node = (is_remote ? WS_NODEID (op) : DB_CLUSTER_NODE_LOCAL);
  ...
  if (btree_find_unique (&btid, value, ws_oid (classop), &unique_oid, node) ==
      BTREE_KEY_FOUND)
    {
      if (!is_remote)
	{
	...
	{
	  ws_set_rmt_oid (op, &unique_oid, DB_CLUSTER_NODE_NULL);
	  found = op;
	}
    }
  ...
  if (op == NULL || desc == NULL || value == NULL)
  ...
  else
    {
      ...
      if (sm_get_descriptor_component (op, desc, 0, &class_,
				       (SM_COMPONENT **) (&att)) == NO_ERROR)
	{
	  obj = find_unique (op, att, value, fetchmode, NULL);
	}
    }
 ...
 *    fetchmode(in): access type    AU_FETCH_READ
 *                              AU_FETCH_UPDATE
 *    inst_op(in): this argument is only used when find a remote unique instance.
 *
 * Note:
...
obj_find_unique (MOP op, const char *attname,
		 DB_VALUE * value, AU_FETCHMODE fetchmode, MOP inst_op)
{
  SM_CLASS *class_;
  SM_ATTRIBUTE *att;
  MOP obj;
  ...
  if (op == NULL || attname == NULL || value == NULL)
	...
	{
	  obj = find_unique (op, att, value, fetchmode, inst_op);
	}
    }
 ...
 *   fetchmode(in): access type AU_FETCH_READ
 *                              AU_FETCH_UPDATE
 *   inst(in/out): this parameter only used when this is a remote request. 
 *                The remote instance oid will be stored in this MOP.
 *
 */
MOP
		     ...
		     MOP inst)
{
  ...
  obj = obj_find_object_by_cons_and_key (op, cons, &key, fetchmode, inst);
		     ...
		     const DB_VALUE * values[], AU_FETCHMODE fetchmode)
{
  ...
  obj = obj_find_object_by_cons_and_key (op, cons, &key, fetchmode, NULL);
 ...
 *    fetchmode(in):    access type    AU_FETCH_READ
 *                                     AU_FETCH_UPDATE
 *    inst (in/out): local instance MOP. only used when process remote instance
 *
 * Note:
 *    This will try to find an object that has a particular value in either
 *    a unique btree or a regular query btree.
				 ...
				 MOP inst)
{
  ...
  is_remote = inst != NULL ? WS_IS_REMOTE (inst) : false;
  node = is_remote ? WS_NODEID (inst) : DB_CLUSTER_NODE_LOCAL;
  ...
  if (btree_find_unique
      (&cons->index, key, ws_oid (classop), &unique_oid,
       node) == BTREE_KEY_FOUND)
    {
      if (!is_remote)
	{
	...
	{
	  if (ws_set_rmt_oid (inst, &unique_oid, DB_CLUSTER_NODE_NULL))
	    {
	      obj = inst;
	    }
	  else
  ...
  if (is_replication == true)
  ...
  else
    {
      btree_search = btree_find_unique (&cons->index, key, ws_oid (classop),
					&unique_oid, DB_CLUSTER_NODE_LOCAL);
    }

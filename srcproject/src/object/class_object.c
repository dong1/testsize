...
classobj_free_template (SM_TEMPLATE * template_ptr)
{
  ...
  ws_free_string (template_ptr->node_name);
...
classobj_make_template (const char *name, MOP op, SM_CLASS * class_)
{
  ...
  template_ptr->is_global = false;
  template_ptr->node_name = NULL;
  OID_SET_NULL (&template_ptr->real_oid);
  HFID_SET_NULL (&template_ptr->real_hfid);
  ...
  if (class_ != NULL)
    {
      ...
      COPY_OID (&template_ptr->real_oid, &class_->real_oid);
      HFID_COPY (&template_ptr->real_hfid, &class_->real_hfid);
      template_ptr->node_name = ws_copy_string (class_->node_name);
      template_ptr->is_global =
	class_->flags & (SM_CLASSFLAG_GLOBAL) ? true : false;
...
classobj_make_class (const char *name)
{
  ...
  OID_SET_NULL (&class_->real_oid);
  HFID_SET_NULL (&class_->real_hfid);
...
classobj_free_class (SM_CLASS * class_)
{
  ...
  ws_free_string (class_->node_name);

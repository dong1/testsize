		...
		MOBJ volatile obj, RECDES * record, bool * index_flag)
{
  ...
  switch (_setjmp (buf->env))
	...
	{
	  or_put_oid (buf, &class_->real_oid);
	}
      else
	{
...
put_class_varinfo (OR_BUF * buf, SM_CLASS * class_)
{
  ...
  or_put_int (buf, offset);
  offset += string_disk_size (class_->node_name);
...
put_class_attributes (OR_BUF * buf, SM_CLASS * class_)
{
  ...
  /* real oid of global class */
  or_put_oid (buf, &class_->real_oid);

  /* real hfid of global class */
  or_put_int (buf, class_->real_hfid.vfid.fileid);
  or_put_int (buf, class_->real_hfid.vfid.volid);
  or_put_int (buf, class_->real_hfid.hpgid);
  ...
  /* node name of global class */
  put_string (buf, class_->node_name);
}
...
tf_class_size (MOBJ classobj)
{
  ...
  size += string_disk_size (class_->node_name);
...
tf_dump_class_size (MOBJ classobj)
{
  ...
  s = string_disk_size (class_->node_name);
  fprintf (stdout, "Node_name%d\n", s);
  size += s;
...
disk_to_class (OR_BUF * buf, SM_CLASS ** class_ptr)
{
  ...
  if (vars == NULL)
      ...
      else
	{
	  ...
	  /* real oid of global object */
	  rc = or_get_oid (buf, &class_->real_oid);

	  /* real heap file id of global object */
	  class_->real_hfid.vfid.fileid = or_get_int (buf, &rc);
	  class_->real_hfid.vfid.volid = or_get_int (buf, &rc);
	  class_->real_hfid.hpgid = or_get_int (buf, &rc);
	  ...
	  /* variable 15 */
	  class_->node_name = get_string (buf, vars[15].length);
...
tf_class_to_disk (MOBJ classobj, RECDES * record)
{
  ...
  switch (rc)
      ...
      else
	{
      ...
      /* chn */
      if (class_->allow_new_chn)
	{
	  chn = class_->header.obj_header.chn + 1;
	  if (class_->flags & SM_CLASSFLAG_GLOBAL)
	    {
	      class_->allow_new_chn = 0;
	    }
	}
      else
	{
	  /* proxy class has the same chn with global class. */
	  chn = class_->header.obj_header.chn;
	}
      or_put_int (buf, chn);

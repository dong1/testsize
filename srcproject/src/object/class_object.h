...
typedef enum
{
  ...
  SM_CLASSFLAG_GLOBAL = 16,	/* the class is a global class */
} SM_CLASS_FLAG;
...
struct sm_class
{
  ...
  const char *node_name;	/* the location where real class stores */
  OID real_oid;			/* the oid of real class on remote node */
  HFID real_hfid;		/* the hfid of real class on remote node */

  unsigned allow_new_rep:1;	/* set when allow this schema to get new repid */
  unsigned allow_new_chn:1;	/* set when allow this schema to get new chn. */
...
struct sm_template
{
  ...
  bool is_global;		/* if this is a global table. */
  const char *node_name;	/* the location where real class stores */
  OID real_oid;			/* the oid of real class on remote node */
  HFID real_hfid;		/* the hfid of real class on remote node */
};
